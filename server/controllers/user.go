...
func (c *UserController) GetEmployeeByEmployeeNumber() {
	var resp structAPI.RespData

	idStr := c.Ctx.Input.Param(":id")
	ID, errCon := strconv.ParseInt(idStr, 0, 64)
	if errCon != nil {
		helpers.CheckErr("Convert id failed @GetEmployeeByEmployeeNumber - controller", errCon)
		resp.Error = errors.New("Convert id failed").Error()
		return
	}

	resGet, errGetReject := logicUser.GetUserByEmployeeNumber(ID)
	if errGetReject != nil {
	...
	err := c.Ctx.Output.JSON(resp, false, false)
	if err != nil {
		helpers.CheckErr("Failed giving output @GetEmployeeByEmployeeNumber - controller", err)
	}
}

// GetListEmployee ...
func (c *UserController) GetListEmployee() {
	var resp structAPI.RespData

	resGet, errGetReject := logicUser.GetListEmployee()
	if errGetReject != nil {
		resp.Error = errGetReject.Error()
		c.Ctx.Output.SetStatus(400)
	} else {
